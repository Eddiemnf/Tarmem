-- =======================================================================================
-- Tarmem — 017: the bid message as a numbered record, the last template Meta's Arabic checker re-filed
--
-- HOW TO RUN: Supabase → SQL Editor → paste this whole file → Run. Safe to run more than once.
--
-- After 016, seventeen of eighteen were accepted as Utility; the Arabic "new bid" was re-filed as Marketing
-- again. What it still had that the accepted Arabic ones did not: "new" and a plural of "bids" — the shape of
-- an offer. Written as a status record — "status update for project no. …: bid number 3 was received from
-- …, 52,000 riyals, 30 days" — it matches the Arabic that passed (the signed-agreement message, which also
-- names a company and an amount). Submitted in both languages under tarmem_new_bid_3; the bid's number on
-- the project is the new second variable.
-- =======================================================================================
...
-- who is told what (as in 014); the bid message now carries the bid number on the project ------------------------
...
select public.wa_delete_template('tarmem_new_bid_2');
select public.wa_submit_templates(p_events := array['new_bid']);
