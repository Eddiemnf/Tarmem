/* 010 (emails to customers and contractors) in a private Postgres, with pg_net imitated. See local.mjs. */
...
await db.exec(`
  ...
  language plpgsql as $$ begin insert into public.sent (to_addr, subject, html) values (body->'to'->>0, body->>'subject', body->>'html'); return 1; end; $$;`);
...
const last = async () => { await db.exec('reset role'); return (await db.query(`select * from public.sent order by id desc limit 1`)).rows[0]; };
