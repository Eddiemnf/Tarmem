...
export default function Footer({ vm }: { vm: VM }) {
  return (<><footer style={{ background: 'linear-gradient(135deg,#FDF3EC 0%,#FCEEE7 48%,#F5EFF7 100%)', color: '#1B1464', marginTop: 'auto' }}>
    ...
    <div className="wrap" style={{ display: 'flex', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap', fontSize: '12px', color: '#5B5A7A', paddingBlock: '14px 20px', borderTop: '1px solid rgba(27,20,100,.12)' }}><span>© {vm.year} {vm.t.brand}{vm.t.footer.rights}</span><span className="num legal-line">{vm.legalLine}</span><span>{vm.t.footer.note}</span></div>
  </footer></>);
}
