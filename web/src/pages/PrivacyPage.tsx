...
export default function PrivacyPage({ vm }: { vm: VM }) {
  return (<>
    <section className="wrap fade" style={{ paddingBlock: '56px 80px', maxWidth: '820px' }}>
      <span className="kick">{vm.t.pages.privacyKicker}</span>
      <h1 style={{ fontSize: 'clamp(30px,3.4vw,44px)', color: '#1B1464', margin: '12px 0 8px' }}>{vm.t.pages.privacyTitle}</h1>
    ...
    {((vm.t.pages.privacy) || []).map((c: any, _i0: number) => (
