...
import { currentAccount, normalizeMobile, signIn, signUp, validEmail, validMobile } from './session';
      ...
      <div style={{ width: '100%', maxWidth: '432px', display: 'flex', flexDirection: 'column', gap: '26px' }}>
        ...
        <p className="muted" style={{ textAlign: 'center', fontSize: '13px', lineHeight: 1.8 }}>
          {signup
            ...
            : <>{copy.forgot} <a className="lnk" data-route="contact" onClick={vm.go} style={{ color: '#FF5A3C' }}>{copy.forgotLink}</a>.</>}
        </p>
      </div>
    </section>
  );
